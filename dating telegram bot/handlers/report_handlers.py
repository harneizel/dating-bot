...
async def report_feedback(message: Message, state: FSMContext):
    await message.answer('Введите в первой строчке id юзера на которого отправили жалобу, а в следующих текст, который он увидит')
...
async def report_feedback_send(message: Message, state: FSMContext, bot: Bot):
    ...
    await bot.send_message(chat_id=report_user_id, text= f'Кто-то пожаловался на вашу анкету, поэтому мы приняли меры:\n{report_text}')
    await message.answer(f'Отлично! Жалоба была отправлена юзеру - {report_user_id}')
    await state.clear()